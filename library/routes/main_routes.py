    ...
    if form.validate_on_submit():
        if request.method == 'POST':
            ...
            if Book.query.filter_by(title=title_returned, author=author_returned).first():
                ...
                return redirect(url_for('book.edit_title', id=book.id, total=total))
                ...
                if len(books) > 1:
                    ...
                    return redirect(url_for('author.bibliography', author=book.author, id=book.id, total=total))
                ...
                if len(books) > 1:
                    ...
                    return redirect(url_for('author.bibliography', author=author_returned, form=form, books=books, total=total))
                elif len(books) == 1:
                    book = Book.query.filter_by(author=author_returned).first()
                    return redirect(url_for('author.bibliography', author=author_returned, id=book.id, total=total))
            elif Book.query.filter_by(isbn=isbn_returned):
                book = Book.query.filter_by(isbn=isbn_returned).first()
                return redirect(url_for('author.bibliography', author=book.author, total=total))
